...
new function() {
	...
	//!add by laowine to force update matrix only
	_updateMatrixOnly: function(param) {
		...
		if(children)
		{
			for (var i = 0, l = children.length; i < l; i++) {
			var item = children[i];
				item._updateMatrixOnly(param);
			}
		}
		matrices.pop();
	},
	//!add by laowine to force update matrix only
	updateMatrixOnly:function(param)
	{
		if(!param)
			param = new Base({
